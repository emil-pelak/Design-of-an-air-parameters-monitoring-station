...
#define DISPLAY_HEIGHT        64
    ...
    void lcd_oled_init(uint8_t dispAttr);
    ...
    void lcd_puts_p(const char* progmem_s);	// print string from flash on screen (TEXTMODE)
    						// or buffer (GRAPHICMODE)
	void oled_int(int val);
	void oled_long_int(long int val);
	void oled_float(float val);
    
    void lcd_clrscr(void);                	// clear screen (and buffer at GRPHICCMODE)
